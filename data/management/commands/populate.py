...
deserunt mollit anim id est laborum."""[init:end]
	...
	def addCategory(self, noCategories):
		# Create category with name categoryx x a number between 0 and argument noCategories
	...
	def addWorkflow(self, noWorkflows):
		...
		for i in range(0, noWorkflows):
			# r is a randint between 0 and (numbre of categories -1)
			...
			w =  Workflow.objects.get_or_create(name="workflow " + str(i) + "-" + str(r),
					...
					json = self.getJson())[0]
			# Add two random categories to the workflow
